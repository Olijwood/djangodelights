...
def recipe_delete_view(request, id=None):
    ...
    if request.method == "POST":
        ...
        if request.htmx:
            ...
            raise Http404("Success", headers=headers)
...
def recipe_ingredient_delete_view(request, parent_id=None, id=None):
    try:
        obj = RecipeIngredient.objects.get(id=id, recipe__user=request.user, recipe__id=parent_id)
    except:
        obj = None
    if obj is None:
        if request.htmx:
    ...
    if request.method == "POST":
        ...
        if request.htmx:
            ...
            raise Http404("Ingredient Removed", headers=headers)
        return redirect(success_url)
    context = {
        "object": obj
    }
